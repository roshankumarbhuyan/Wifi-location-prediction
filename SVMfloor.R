...
#====================svm BuildingID============
set.seed(2334)
trctrl <- trainControl(method = "cv", number = 5,verbose = FALSE)
svm_tune <- train(FLOOR ~., data = trainSetflr, trControl=trctrl, method= "svmlinear")
svm_tune
svmpred = predict(svm_tune, newdata=testSetflr, trControl=trctrl,
                  preProcess = c("center", "scale"),tuneLength = 8)
threshold <- 0.5
postResample(svmpred, testSetlat$LATITUDE)
saveRDS(svmpred, file = "SVMfloorID")
my_model2 <- readRDS("SVMfloorID")
